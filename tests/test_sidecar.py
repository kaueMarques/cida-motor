...
from markdown.sidecar import (
    ...
    SidecarValidationError, calculate_sha256
)
...
class TestSidecarDictionaries(unittest.TestCase):
    def setUp(self):
        ...
        self.entries = [
            {"alias": "AA", "value": "implementation"},
            {"alias": "BB", "value": "content"}
        ]
    ...
    def test_create_and_validate_sidecar_success(self):
        data = create_sidecar_data("test.md", self.source_content, self.entries)
        # Should validate without throwing any error
        validate_sidecar_schema(data)
        
        self.assertEqual(data["format"], "cida-token-dictionary")
        self.assertEqual(data["version"], 1)
        self.assertEqual(data["source"], "test.md")
        self.assertEqual(data["source_sha256"], calculate_sha256(self.source_content))
        self.assertEqual(data["tokenizer"], "cl100k_base")
        self.assertEqual(len(data["entries"]), 2)
        
        # Verify entries are sorted by alias
        self.assertEqual(data["entries"][0]["alias"], "AA")
        self.assertEqual(data["entries"][1]["alias"], "BB")
        
    def test_duplicate_alias_throws(self):
        bad_entries = [
            {"alias": "AA", "value": "first"},
            {"alias": "AA", "value": "second"}
        ]
        with self.assertRaises(SidecarValidationError):
            create_sidecar_data("test.md", self.source_content, bad_entries)
            
    def test_duplicate_value_throws(self):
        bad_entries = [
            {"alias": "AA", "value": "common"},
            {"alias": "BB", "value": "common"}
        ]
        with self.assertRaises(SidecarValidationError):
            create_sidecar_data("test.md", self.source_content, bad_entries)
    ...
    def test_compress_decompress_roundtrip(self):
        ...
        if sidecar_data:
            mapping = {entry["alias"]: entry["value"] for entry in sidecar_data["entries"]}
